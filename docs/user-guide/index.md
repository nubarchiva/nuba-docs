...
```mermaid
graph LR
    A[Ir a Búsqueda] --> B[Ingresar términos]
    B --> C[Aplicar filtros]
    C --> D[Ver resultados]
...
```mermaid
graph LR
    A[Buscar documentos] --> B[Añadir a colección]
    B --> C[Repetir búsqueda]
    C --> D[Organizar colección]
...
```mermaid
graph LR
    A[Localizar documento] --> B[Ver detalle]
